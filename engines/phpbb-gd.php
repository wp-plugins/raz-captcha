...
*
* @package VC
* @version $Id: captcha_gd.php,v 1.22 2007/05/06 16:14:42 kellanved Exp $
...
class captcha
{
	var $width = 320;
	var $height = 96;
    var $bg_images = array("engines/razcap1.jpg","engines/razcap2.jpg");
	/**
	...
	*/
	function execute($code, $seed)
	{
        ...
        $config['captcha_gd_x_grid']=mt_rand(40, 50);
        $config['captcha_gd_y_grid']=mt_rand(50, 70);
        $config['captcha_gd_foreground_noise']=mt_rand(50, 70);
 		// Create image
	...
	function imgChunks ($imgc)
	{
		...
		/*	$blocksize = mt_rand(20,60);
			for($i=0 ; $i<$width*2 ; $i+=$blocksize)
			{
				...
				{
					$image_index = mt_rand(0,count($temp_im)-1);
					$cut_x = mt_rand(0,$temp_width[$image_index]-$blocksize);
					$cut_y = mt_rand(0,$temp_height[$image_index]-$blocksize);
					ImageCopy($img, $temp_im[$image_index], $i, $j, $cut_x, $cut_y, $blocksize, $blocksize);
				}
			}
